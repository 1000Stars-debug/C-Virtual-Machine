...
class VisualAssemblyEditor:
    def __init__(self, root):
        ...
        self.current_rgb565 = 0

        self.build_topbar()
        self.sidebar_frame = tk.Frame(self.root, width=300, bg="#333")
        ...
        self.color_tab = tk.Frame(self.notebook, bg="#333")
    ...
    def build_color_picker(self):
        tk.Label(self.color_tab, text="RGB888 to RGB565", bg="#333", fg="white", font=("Arial", 11, "bold"), pady=10).pack()
        
        # Preview Box
        self.color_preview = tk.Frame(self.color_tab, width=100, height=100, bg="black", highlightthickness=2, highlightbackground="#555")
        self.color_preview.pack(pady=10)
        self.color_preview.pack_propagate(False)

        # Labels
        self.rgb_lbl = tk.Label(self.color_tab, text="RGB: (0, 0, 0)", bg="#333", fg="#AAA", font=("Consolas", 10))
        self.rgb_lbl.pack()
        self.hex_lbl = tk.Label(self.color_tab, text="HEX: #000000", bg="#333", fg="#AAA", font=("Consolas", 10))
        self.hex_lbl.pack()
        self.dec_lbl = tk.Label(self.color_tab, text="DEC: 0", bg="#333", fg="#A2D149", font=("Consolas", 12, "bold"))
        self.dec_lbl.pack(pady=5)

        # Buttons
        tk.Button(self.color_tab, text="Pick Color Wheel", command=self.pick_system_color, bg="#4DB6AC", relief="flat", padx=10).pack(pady=10)
        
        tk.Label(self.color_tab, text="Quick Inject:", bg="#333", fg="#888", font=("Arial", 9, "italic")).pack(pady=(10, 0))
        tk.Button(self.color_tab, text="Spawn PUSH [Color]", command=self.spawn_color_block, bg="#4FC3F7", relief="flat", font=("Arial", 9, "bold")).pack(pady=5, fill="x", padx=40)

    def pick_system_color(self):
        color = colorchooser.askcolor(title="CVM Color Picker")
        if color[0]:
            ...
            g6 = (g >> 2) & 0x3F
            b5 = (b >> 3) & 0x1F
            self.current_rgb565 = (r5 << 11) | (g6 << 5) | b5

            # Update UI
            self.color_preview.configure(bg=hex_str)
            self.rgb_lbl.configure(text=f"RGB: ({r}, {g}, {b})")
            self.hex_lbl.configure(text=f"HEX: {hex_str.upper()}")
            self.dec_lbl.configure(text=f"DEC: {self.current_rgb565}")

    def spawn_color_block(self):
        self.spawn_block_with_value("PUSH", "Core", True, str(self.current_rgb565))

    def spawn_block_with_value(self, opcode, category, has_input, value):
        ...
        if has_input:
            ...
            new_block.entry.insert(0, value)
        
        self.blocks.append(new_block)
        self.update_connections()
        self.save_history_state()

    # --- Original Workspace Methods ---
    ...
    def spawn_block(self, opcode, category, has_input):
        self.spawn_block_with_value(opcode, category, has_input, "0")
    ...
    def update_connections(self):
        ...
        for i in range(len(sorted_b) - 1):
            ...
            dy = cy2 - cy1
            if abs(dx) > abs(dy):
                if dx > 0: sx, sy, ex, ey = c1[2], cy1, c2[0], cy2
                else: sx, sy, ex, ey = c1[0], cy1, c2[2], cy2
            else:
                if dy > 0: sx, sy, ex, ey = cx1, c1[3], cx2, c2[1]
                else: sx, sy, ex, ey = cx1, c1[1], cx2, c2[3]
    ...
    def compile_script(self):
        ...
        try:
            with open(out_path, "w") as f:
                f.write(full_output)
            messagebox.showinfo("Compiled", f"Successfully compiled to: {os.path.basename(out_path)}")
        except Exception as e:
