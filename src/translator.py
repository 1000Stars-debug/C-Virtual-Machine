...
class BytecodeCompiler:
    ...
    def compile_file(self, input_filepath, output_filepath):
        ...
        # Instructions that require a 16-bit argument (3 bytes total)
        ...
        for line_idx, line in enumerate(self.instructions):
            ...
            if cmd in arg_instructions:
                ...
                # FIX: Force Python to handle negative numbers via Two's Complement
                val = val & 0xFFFF 
...
if __name__ == "__main__":
    # FIX: Check sys.argv safely to prevent crashes
    if len(sys.argv) < 2:
    ...
    # If they provided an output name, use it. Otherwise, default to main.cvm
    if len(sys.argv) == 3:
        exec_name = sys.argv[2]
    else:
        exec_name = "main.cvm"
        
    BytecodeCompiler().compile_file(source_file, exec_name)
